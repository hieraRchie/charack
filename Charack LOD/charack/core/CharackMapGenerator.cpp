...
void CharackMapGenerator::applyCoast(int theMapX, int theMapZ, int theViewFrustum, int theSample) {
	...
	// Here we need to find all coast lines in the screen (which is rendered using the "coast map", a matrix 
	// that informs what is land and what is water). All Charack continents are squared-pieces,so all coast
	// lines are straight lines (horizonal or vertical ones). The screen should look like this:
	//
	//   --------------------------------  -
	//   |       E------------D         |  |
	//   |       |            |         |  |
	//   |   A---F            |         |  |
	//   |   |       Land     |         |  |- Coast map height
	//   |   |                |         |  |
	//   |   B----------------C         |  | 
	//   |               Water          |  |
	//   --------------------------------  -
	...
	//           Coast map width
	//
	// A, B, etc are continent corners. The findCoastLines() will return a set of line segments like A-B, B-C, C-D,
	// D-E, E-F nad F-A.
	// After we have the segmnets, we preceed with the for() below: for each segment, we apply some noise and
	// create a "real" coast line. After that, we plot all noised lines inside the coast map, which will be used
	// by all other Charack methods to identify what is land and what is water in the screen.
